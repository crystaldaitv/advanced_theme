...
if (!function_exists('advancedThemeLogo')) {
    ...
    function advancedThemeLogo() {?>
        <div class="logo_container logo_global">
            <?php if ( is_home() ) {
                printf('<div class="logo-content"><h1 class="logo"><a id="logo-link" class="logo-link" href="%1$s" title="%2$s"></a></h1></div>',
                    get_bloginfo( 'url' ),
                    get_bloginfo( 'description' )
                );
                printf('<div class="site-info-content"><h1 class="site-title"><a id="site-link" class="site-link" href="%1$s" title="%2$s">%3$s</a></h1><p class="site-tagline">%4$s</p></div>',
                    get_bloginfo( 'url' ),
                    get_bloginfo( 'name' ),
                    get_bloginfo( 'name' ),
                    get_bloginfo( 'description' )
                ...
                printf('<div class="logo-content"><p class="logo"><a id="logo-link" class="logo-link" href="%1$s" title="%2$s"></a></p></div>',
                    get_bloginfo( 'url' ),
                    get_bloginfo( 'description' )
                );
                printf('<div class="site-info-content"><p class="site-title"><a id="site-link" class="site-link" href="%1$s" title="%2$s">%3$s</a></p><p class="site-tagline">%4$s</p></div>',
                    get_bloginfo( 'url' ),
                    get_bloginfo( 'name' ),
                    get_bloginfo( 'name' ),
                    get_bloginfo( 'description' )
     ...
     * Theme menu function
     ...
     function advancedThemeMenu( $slug ) {
        $menu = array(
            ...
            'items_wrap'      => '<ul id="%1$s" class="%2$s advanced-theme-menu">%3$s</ul>',
        );
        wp_nav_menu( $menu );
     }
 }
    ...
    function advancedThemeEntryHeader() {
        if ( is_single() ) : ?>
            <h1 class="entry-title">
                <a class="post-link" href="<?php the_permalink(); ?>" rel="bookmark" title="<?php the_title_attribute(); ?>"><?php the_title(); ?></a>
            </h1>
        <?php else : ?>
    ...
    function advancedThemeComment($comment, $args, $depth)
    {
        ...
        switch ($comment->comment_type) {
            ...
            case 'trackback': ?>
                ...
                <p>
                    <?php esc_attr_e('Pingback:', 'advanced-theme'); ?><?php comment_author_link(); ?><?php edit_comment_link(__('Edit', 'advanced-theme'), '<span class="edit-link">', '<span>'); ?>
                </p> <?php
                break;
            default: ?>
                ...
                <article id="comment-<?php comment_ID(); ?>" class="comment-inner">
                    <div class="flex-row align-top">
                        ...
                        <div class="flex-col flex-grow">
                            <?php printf(__('%s <span class="says">says:</span>', 'advanced-theme'), sprintf('<cite class="strong fn">%s</cite>', get_comment_author_link())); ?>
                            <?php if ((int)$comment->comment_approved === 0) : ?>
                                <em><?php esc_attr_e('Your comment is awaiting moderation.', 'advanced-theme'); ?></em><br/>
                            <?php endif; ?>
                            ...
                            <div class="comment-meta commentmetadata uppercase is-xsmall clear">
                                <a href="<?php echo esc_url(get_comment_link($comment->comment_ID)); ?>">
                                    <time datetime="<?php comment_time('c'); ?>" class="pull-left">
                                        <?php printf(_x('%1$s at %2$s', '1: date, 2: time', 'advanced-theme'), get_comment_date(), get_comment_time()); ?>
                                    </time>
                                </a>
...
if (!function_exists('advancedThemeOptionSettingCallback')) {
    ...
    function advancedThemeOptionSettingCallback()
    {
        ...
        if (isset($customized)) {
            ...
            // Global posts per page handler
            if (isset($customized['postsPerPage'])) {
                update_option('posts_per_page', intval($customized['postsPerPage']));
            }
        }
