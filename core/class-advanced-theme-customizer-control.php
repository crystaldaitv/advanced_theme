...
/**
 * Class AdvancedThemeCustomizeControl
 */
class AdvancedThemeCustomizeControl extends WP_Customize_Control
{
    /**
     * Type
     *
    ...
    public function enqueue() {
        wp_enqueue_script( 'advanced-theme-customizer-control', THEME_URL . '/assets/js/customizer-controls-js.js', array( 'jquery' ), rand(), true );
        wp_enqueue_style( 'advanced-theme-customizer-control', THEME_URL . '/assets/css/customizer-controls-styles.css', array(), rand() );
    }
    ...
    { ?>
        ...
        foreach ($this->choices as $value => $label) :
            ...
            <span class="ag_select_type <?php echo esc_attr($this->icon); ?> <?php echo esc_attr($checked_class); ?> <?php echo esc_attr($label); ?>" data-value="<?php echo esc_attr($label); ?>">
                    <?php if ($this->icon !== 'none') :
                        echo esc_attr($value);
                    endif; ?>
            </span> <?php
        endforeach; ?>
        <input type="hidden" class="ag_value_option" <?php $this->input_attrs(); ?> value="<?php echo esc_attr($this->value()); ?>" <?php $this->link(); ?> /> <?php
    }
    ...
    public function selectFooterColumnsRenderContent()
    {
        ...
        <ul class="ag_theme_select"> <?php
            if (empty($this->choices)) {
                return;
            }
            foreach ($this->choices as $value => $label) :
                $checked_class = (string) $value === (string) $current_values ? ' ag_theme_selected' : ''; ?>
                <li class="ag_theme_option ag_theme_option_<?php echo esc_attr($value); ?> <?php echo esc_attr($checked_class); ?>" data-value="<?php echo esc_attr($value); ?>"></li> <?php
            endforeach; ?>
        </ul>
        <input type="hidden" class="ag_value_option" <?php $this->input_attrs(); ?> value="<?php echo esc_attr($current_values); ?>" <?php $this->link(); ?> /> <?php
    }
    ...
    public function checkboxRenderContent()
    {
        ...
        <span class="customize-inside-control-row">
            <input id="_customize-input-<?php echo esc_attr($this->id);?>" type="checkbox" <?php $this->input_attrs();?> value="<?php echo esc_attr($current_values); ?>" <?php $this->link(); ?> />
    ...
    public function selectFontOptionRenderContent()
    {
        ...
        <div class="ag_theme_select_span ag_theme_select_option"> <?php
            if ($current_values === '') {
        ...
        <ul class="ag_theme_select">
            ...
            <li data-value="" class="ag_theme_option ag_theme_option_default"><?php echo esc_attr__('Default Theme Font', 'advanced-gutenberg-theme'); ?></li> <?php
            if (empty($this->choices)) {
                return;
            }
            foreach ($this->choices as $value => $label) :
                ...
                <li class="ag_theme_option ag_theme_option_<?php echo esc_attr($value); ?> <?php echo esc_attr($checked_class); ?>"
                    data-value="<?php echo esc_attr($value); ?>"><?php echo esc_attr($value); ?></li> <?php
            endforeach;
            ?>
        </ul>
        <input type="hidden" class="ag_value_option " <?php $this->input_attrs(); ?> value="<?php echo esc_attr($current_values); ?>" <?php $this->link(); ?> /> <?php
    ...
    public function selectOptionRenderContent()
    {
        ...
        <ul class="ag_theme_select <?php echo esc_attr($class[1]); ?>">
            ...
            foreach ($this->choices as $key => $value) :
                $checked_class = (string) $value === (string) $current_values ? ' ag_theme_selected' : ''; ?>
                <li class="ag_theme_option ag_theme_option_<?php echo esc_attr($value); ?> <?php echo esc_attr($checked_class); ?>"
                    data-key="<?php echo esc_attr($key); ?>" data-value="<?php echo esc_attr($value); ?>"><?php echo esc_attr($value); ?></li> <?php
            endforeach; ?>
        </ul>
        <input type="hidden" class="ag_value_option " <?php $this->input_attrs(); ?> value="<?php echo esc_attr($current_values); ?>" <?php $this->link(); ?> /> <?php
    }
